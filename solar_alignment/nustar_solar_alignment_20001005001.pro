...
.compile $NUSTAR_SSWPATH/nustar_offset_dbase_gen
.compile $NUSTAR_SSWPATH/nustar_make_submap

;;;;; Set the following path and file info to be appropriate to what
;;;;; you want to run.
indir = '/users/bwgref/lif_nustar/sol/data/20001005_Sol_14345_AR2222/20001005001/event_cl'

infile = 'nu20001005001A06_cl.evt'
...
nustar_get_reference_image, indir+'/'+infile, outdir = datdir, $
                            outfile = ref_file, wave = 171


; Align each tile to the reference tile

map_files= file_search(mapdir+'/nu'+seqid+'*sunpos_map.fits')
; Here we have to tinker by hand for this one. Know that i=0 is FPMA, i = 1 is FPMA:
; First pair is for FPMA, second pair is for FPMB
shift_manual = [ [5, 20], [40, 50]]
FOR i = 0, n_elements(map_files) -1 DO nustar_align_tile, ref_file,map_files[i],datdir = datdir, figdir = figdir, fov=20, levels = [1, 10, 20, 30, 40], shift_manual = shift_manual[*, i]
